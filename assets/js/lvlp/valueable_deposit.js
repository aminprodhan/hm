$(document).ready(function() {
    $('#deposit_return_date,#valueable_deposit_search_from_date,#valueable_deposit_search_to_date').datepicker({
        todayBtn: "linked",
...
$(document).on("focusin", "#deposit_return_date,#laundry_bill_search_to_date", function() {
    $(this).prop('readonly', true);
});

$(document).on("focusout", "#deposit_return_date,#laundry_bill_search_to_date", function() {
...
$('#valueable_deposit_item_quantity').bootstrapNumber();
...
function quantity_calcultaion() {
    var total;
    var deposit_item_quantity = $("#valueable_deposit_item_quantity").val();
    total = (Number(deposit_item_quantity));
    $("#valueable_deposit_balance").val(total);
}
...
function addNewDeposit() {
    var deposit_no = $("#deposit_no").val();
    var deposit_check_in_no = $("#valueable_deposit_check_in_no").val();
    var deposit_item_select = $("#valueable_deposit_item_select").val();
    var deposit_item_quantity = $("#valueable_deposit_item_quantity").val();
    var deposit_balance = $("#valueable_deposit_balance").val();
    var deposit_remarks = $("#valueable_deposit_remarks").val();

    if (deposit_no != '' && deposit_no != 0
        && deposit_check_in_no != '' && deposit_check_in_no != 0
        && deposit_item_select != '' && deposit_item_select != 0 ) {

        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: li + 'lvlp/addNewDeposit',
            data: {
                deposit_no: deposit_no,
                deposit_check_in_no: deposit_check_in_no,
                deposit_item_select: deposit_item_select,
                deposit_item_quantity: deposit_item_quantity,
                deposit_balance: deposit_balance,
                deposit_remarks: deposit_remarks,
            },
            success: function (data) {
                if (data.id == 2) {
                    //alert('new inserted into two table');
                    $("#valueable_deposit_check_in_no").attr("disabled", true);
                    $('#valueable_deposit_check_in_no').selectpicker('refresh');
                    $('#valueable_deposit_item_list').trigger("reset");
                    ...
                    item_list(deposit_no);
                }
                else if (data.id == 1) {
                    alert($("#deposit_return_date").val());
                    item_list(deposit_no);
                    $('#valueable_deposit_item_list').trigger("reset");
        ...
        if (deposit_check_in_no == '' && deposit_check_in_no == 0) {
        ...
        }else if(deposit_item_select == '' && deposit_item_select == 0){
...
function item_list(deposit_no,show_delete_btn = 1) {
    if (deposit_no) {
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: li + 'lvlp/getDepositItem',
            ...
            success: function (data) {
                ...
                $.each(data.info, function (key, val) {
                    ...
                    stuff = stuff + "<tr class='text-center'>"
                        + "<td>" + val.item_no + "</td>"
                        + "<td>" + val.quantity + "</td>"
                        + "<td><input type='text' value='" + val.return_quantity + "' class='custom_input' id='deposit_return_quantity'></td>"
                        + "<td><input type='text' value='" + val.return_date + "' class='custom_input' id='deposit_return_date'></td>"
                        + "<td>" + val.balance + "</td>"
                        + "<td>" + val.remarks + "</td>"
                    if(show_delete_btn==1){
                        stuff = stuff + "<td><a onclick = delete_laundry_item('"+val.id+"','"+val.voucher_no+"')><i class='fa fa-trash fa-2x' style='color: red'></i></a></td>";
                    }else{
                        stuff = stuff + "<td> </td>";
                    }
                    stuff = stuff + "</tr>";
